...
//
// Used by /api/review. Takes the first-pass audit, performs a
// confirmatory check against a defined depth standard, and outputs a
// single COMPLETE, REVISED audit.
//
// The prompt is verbatim from the operator's spec. Three substitutions
// happen: the gold-standard reference, the first-pass audit, and the
// (optional) location + supporting-report contents.
//
// To refine wording: edit the literal text below.
// To refine the depth standard: edit the gold-standard reference in
// lib/prompts/gold-standard.js.
...
export function buildReviewPrompt({ firstPassAudit, location, reports }) {
  ...
  const locationBlock = location
    ? `\n\nRESTAURANT LOCATION (for the §6 45-mile-radius competitive analysis): ${location}`
    : '';

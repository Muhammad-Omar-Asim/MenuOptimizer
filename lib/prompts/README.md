...
| `basic-analysis-prompt.js` | Single-pass audit prompt. `/api/analyze` swaps to it when the request body has `mode: 'basic'` — triggered by the **Basic Analysis** button, which is auto-suggested when the user uploads a menu JSON without supporting reports. Covers total counts, miscategorisation, menu gaps (combos / deals / upsells / visuals / placement), item-placement recommendations, revenue-generating upsells, meal-deal / off-peak / repeat-customer offers, plus a 5-mile-radius competitor + consumer-behaviour read using the provided location. Returns `buildBasicAnalysisPrompt({ menuJson, location })`. |

...
| `review-prompt.js` | Prompt sent by `/api/review` when the user has the "Run confirmatory check" toggle on. The new spec is a standalone audit — it doesn't reference the first-pass audit content. Returns `buildReviewPrompt({ menuJson, location, reports, firstPassAudit })`; `location` and `firstPassAudit` are accepted for call-site compatibility but not referenced in the prompt body. |
